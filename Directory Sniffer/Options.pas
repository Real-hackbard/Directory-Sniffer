...
begin
  ...
  // For other drive letters, simply modify TempStr.

  //TempStr := Caption;
  //Label1.Caption := 'Laufwerk: ' + TempStr;

  // Query only makes sense for virtual/physical drives.
  Temp := TLaufwerkstyp(GetDriveType(PChar(TempStr)));

  if Temp <> dtFixed then
    ShowMessage('No virtual / physical hard disk!')
  else
    begin
      // Using DeviceIoControl, assign the number(s) to a drive letter.
      // PHYSICALDRIVEn and the partition number on the respective
      // Identify the drive.
      TempStr := '\\.\' + TempStr;
      ...
      if DevIoHandle = Invalid_Handle_Value then
        ShowMessage('Error in CreateFile!')
      else
        begin
      ...
      if not Ergebnis then
        ShowMessage('Error in DeviceIoControl!')
      else
        begin
            ...
            begin
              if FWbemObject.DeviceID = TempStr then
                begin
                  ...
                  // Virtual drives do not have a factory serial number!
